...
export function Drawer({ open, onClose, title, subtitle, width = 'md', children, headerExtra, noPadding }: DrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-[300] pointer-events-none">
      ...
      <div
        ref={ref}
        className={`absolute inset-x-2 bottom-[calc(env(safe-area-inset-bottom)+16px)] max-h-[calc(88vh-env(safe-area-inset-bottom)-16px)] md:inset-x-auto md:top-3 md:right-3 md:bottom-3 md:left-auto md:max-h-none w-auto ${WIDTHS[width]} bg-[var(--surface)] border border-[var(--border)] rounded-[24px] pointer-events-auto flex flex-col anim-sheet overflow-hidden`}
