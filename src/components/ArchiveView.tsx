...
import { Archive, Trash2, CheckCircle2, Search, Clock, RotateCcw } from 'lucide-react';
import { format } from 'date-fns';

export function ArchiveView() {
  const t = useT();
  const locale = useDateLocale();
  const { gtdTasks, processTask } = useStore();
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg)] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 max-w-5xl mx-auto w-full space-y-3">
        {filtered.map((task) => (
          <div key={task.id} className="card p-4 group flex items-start gap-4">
